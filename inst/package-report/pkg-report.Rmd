...
params:
  pkgdir: E:/R_Other/packages/viridisLite
---
...
```{r rev_calls, echo = FALSE, comment = ""}
res <- pkgreviewr::pkg_fn_summary(params$pkgdir)
...
Are the default arguments consistent across all the functions? First, across all functions (exported and non-exported):

```{r rev_args, echo = FALSE, comment = ""}
DT::datatable(pkgreviewr::rev_args(params$pkgdir)$arg_df)
```

Then only for the exported functions:

```{r rev_args_exported, echo = FALSE, comment = ""}
DT::datatable(pkgreviewr::rev_args(params$pkgdir, exported_only = TRUE)$arg_df)
```
